...
- [x] Phase 2 BUILT (29 Jul 2026) — awaiting Ian's ~15-min server go-live (`app/server/SETUP_SERVER.md`). App now has PocketBase sync: `app/src/sync.js` (SDK wrapper), login modal, sync pill (○ Sign in / ● Synced / ● Saving / ● Retry), debounced push (900ms), pull on load + visibilitychange, offline = localStorage fallback, sign-out keeps local copy. Schema: `snapshots` (1 record/user, whole state, last-write-wins) + `weekly_goals` rows (owner+week unique) as progress history. Server kit in `app/server/`: `setup-droplet.sh` (cloud-init: PocketBase 0.28.4 + Caddy auto-HTTPS + systemd + ufw + nightly 03:15 backup keeping 14d), `Fetch-KonqrBackup.ps1` (Windows scheduled task → OneDrive versioned zip; backup API flow validated), migration file. E2E tested: two isolated browsers synced task + mission edits through a real local PocketBase; fixed browser-HTTP-cache stale-pull bug (cache-busting _ts param) and mount-time push-before-pull race. Zero console errors.
